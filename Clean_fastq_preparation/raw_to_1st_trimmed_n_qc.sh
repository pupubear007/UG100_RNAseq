...
echo "=== RNA-seq Quality Control and Trimming Workflow ==="
...
TRIMMED_DIR="${BASE_DIR}/data/trimmed_fastq"
...
# Count samples
TOTAL_SAMPLES=$(ls -1 "${FASTQ_DIR}"/*_R1.fastq.gz 2>/dev/null | wc -l)
...
trim_sample() {
    local r1_file=$1
    local base_name=$(basename "${r1_file}" _R1.fastq.gz)
    local r2_file="${FASTQ_DIR}/${base_name}_R2.fastq.gz"
...
parallel -j 3 ::: \
    "multiqc qc_reports/before_trimming/ -o qc_reports/ -n before_trimming_multiqc --title 'Before Trimming' --force -q" \
    "multiqc qc_reports/after_trimming/ -o qc_reports/ -n after_trimming_multiqc --title 'After Trimming' --force -q" \
    "multiqc ${LOG_DIR}/trimmomatic/ -o qc_reports/ -n trimming_stats_multiqc --title 'Trimming Statistics' --force -q"
