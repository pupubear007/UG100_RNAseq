...
CLEANED_DIR="${BASE_DIR}/data/1cleaned_reads_re"
...
PROBLEM_SAMPLES=($(find "${CLEANED_DIR}" -name "*_R1_cleaned.fastq.gz" -exec basename {} \; | sed 's/_R1_cleaned.fastq.gz//'))
...
for sample in "${PROBLEM_SAMPLES[@]}"; do
    R1_FILE="${CLEANED_DIR}/${sample}_R1_cleaned.fastq.gz"
    R2_FILE="${CLEANED_DIR}/${sample}_R2_cleaned.fastq.gz"
...
for sample in "${PROBLEM_SAMPLES[@]}"; do
    echo ""
    echo "Sample: ${sample}"
    validate_fastq "${CLEANED_DIR}/${sample}_R1_cleaned.fastq.gz"
    validate_fastq "${CLEANED_DIR}/${sample}_R2_cleaned.fastq.gz"
done
...
for sample in "${PROBLEM_SAMPLES[@]}"; do
    echo ""
    find_bad_reads "${CLEANED_DIR}/${sample}_R1_cleaned.fastq.gz"
    find_bad_reads "${CLEANED_DIR}/${sample}_R2_cleaned.fastq.gz"
done
...
repair_fastq() {
    local sample=$1
    local r1_in="${CLEANED_DIR}/${sample}_R1_cleaned.fastq.gz"
    local r2_in="${CLEANED_DIR}/${sample}_R2_cleaned.fastq.gz"
...
for sample in "${PROBLEM_SAMPLES[@]}"; do
    ...
    if [ -f "${REPAIRED_R1}" ]; then
        echo "# For ${sample}:"
        echo "ln -sf ${REPAIRED_R1} ${CLEANED_DIR}/${sample}_R1_cleaned_FIXED.fastq.gz"
        echo "ln -sf ${REPAIRED_R2} ${CLEANED_DIR}/${sample}_R2_cleaned_FIXED.fastq.gz"
        echo ""
    fi
done
